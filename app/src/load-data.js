...
const GCS_PREFIX =
  "https://storage.googleapis.com/japan-railways-data/accesibility";
...
export default async function loadData(city) {
  ...
  const edgesText = await fetch(`${GCS_PREFIX}/edges-caba.csv`).then((res) =>
    res.text()
  );
  const nodesText = await fetch(`${GCS_PREFIX}/nodes-caba.csv`).then((res) =>
    res.text()
  );
